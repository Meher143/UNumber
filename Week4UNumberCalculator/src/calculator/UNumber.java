...
public class UNumber implements Comparable<UNumber> {
	 ...
	 * to compensate.
	 */
	static Double[] preCalculationofSQRT = new Double[]{0.0,1.0,1.4142135623730951,1.7320508075688772,2.0,2.23606797749979,
			2.449489742783178,2.6457513110645907,2.8284271247461903,3.0,3.1622776601683795,3.3166247903554,
			3.4641016151377544,3.605551275463989,3.7416573867739413,3.872983346207417,4.0,4.123105625617661,
			4.242640687119285,4.358898943540674,4.47213595499958,4.58257569495584,4.69041575982343,
			4.795831523312719,4.898979485566356,5.0,5.0990195135927845,5.196152422706632,5.291502622129181,
			5.385164807134504,5.477225575051661,5.5677643628300215,5.656854249492381,5.744562646538029,
			5.830951894845301,5.916079783099616,6.0,6.082762530298219,6.164414002968976,6.244997998398398,
			6.324555320336759,6.4031242374328485,6.48074069840786,6.557438524302,6.6332495807108,6.708203932499369,
			6.782329983125268,6.855654600401044,6.928203230275509,7.0,7.0710678118654755,7.14142842854285,7.211102550927978,
			7.280109889280518,7.3484692283495345,7.416198487095663,7.483314773547883,7.54983443527075,7.615773105863909,
			7.681145747868608,7.745966692414834,7.810249675906654,7.874007874011811,7.937253933193772,8.0,8.06225774829855,
			8.12403840463596,8.18535277187245,8.246211251235321,8.306623862918075,8.366600265340756,8.426149773176359,
			8.48528137423857,8.54400374531753,8.602325267042627,8.660254037844387,8.717797887081348,8.774964387392123,
			8.831760866327848,8.888194417315589,8.94427190999916,9.0,9.055385138137417,9.1104335791443,9.16515138991168,
			9.219544457292887,9.273618495495704,9.327379053088816,9.38083151964686,9.433981132056603,9.486832980505138,
			9.539392014169456,9.591663046625438,9.643650760992955,9.695359714832659,9.746794344808963,9.797958971132712,
	...
	public UNumber(String str, int dec, boolean sign, int size){
		...
		System.out.println("UN " + Arrays.toString(d));
		// The characteristic and sign are exactly the same as the parameters
		dP = dec;
		s = sign;
	}
	
	public UNumber(String str){
		// Use the length of the string to determine the size of the mantissa
		if (str.equals("0")) {
			d = new byte[20];
			for (int i = 0; i < 20; i++) d[i] = 0;
			...
			if(tempdec==-1) {
				str+=".0";
				if(tempe!=-1){
					String tempexp = ((int)(Double.parseDouble(str.substring(tempe+1,str.length())))+1)+"";
					str = str.substring(0,tempe);
					str += "e";
					str += tempexp;
				}
			}
			
			if((tempe==-1)&&(tempdec!=-1)&&(str.charAt(str.length()-1)!='0')) {
				str+="0";
			}
			if((tempe!=-1)&&(tempdec==-1))
			{
				String t="";
				for(int i=0;i<tempe;i++) t+=str.charAt(i);
				t+=".0";
				for(int i=tempe;i<str.length();i++) t+=str.charAt(i);
				str = t;
			}
			...
			if(efound) {
				for(int i=epos+1;i<strlen;i++){
					eVal+=str.charAt(i);
				}
				// System.out.println(eVa);
				eValue = (int)(Double.parseDouble(eVal));
			...
			if(countZ!=0){
				str = str.substring(countZ,str.length());
				eValue += -countZ;
			}
			...
			if(efound){
				// System.out.println("epos: "+epos+" decpos: "+decpos);
				if(decpos!=-1&&tempdec!=-1) eValue += (decpos);
	
				// System.out.println(eValue);
				// this.mpy(new UNumber(eValue));
			}
	
			// System.out.println("decpos: "+decpos+"\nepos: "+epos+"\ncountZ: "+countZ+"\nstr: "+str+"\nevalue: "+eValue);
			// The characteristic and sign are exactly the same as the parameters
			if(decpos!=-1 || str.charAt(0)=='0'){
				if(!efound) dP+=eValue+decpos;
				else dP = eValue;
...
private int numSignificantDigits = 40;
	
	private int howManyDigitsMatch(UNumber newGuess, UNumber oldGuess) {
		...
		// Compute the shorter of the two
	...
	public void sqrt() {
		...
		UNumber newGuess = new UNumber();
		if ((getDouble()+"").length() > 0) {
			Scanner value = new Scanner(getDouble()+"");
			...
			// As long as there is another double value, compute the square root of it
				...
				newGuess = improvement(theValue.toString().substring(1));
				...
				do {
					...
					UNumber q = new UNumber(theValue, numSignificantDigits);
					q.div(oldGuess);
					q.add(oldGuess);
					q.div(two);
					newGuess = q;
				...
				// UNumber resultSquared = new UNumber(newGuess, numSignificantDigits);
				// resultSquared.mpy(newGuess);
				// UNumber estSquared = new UNumber(est, numSignificantDigits);
				// resultSquared.mpy(newGuess);
		...
		this.d = newGuess.d;
		this.dP = newGuess.dP;
		this.numSignificantDigits = newGuess.numSignificantDigits;
				
	}
	
	public UNumber improvement(String num) {
		UNumber un = new UNumber(num);
		String[] nums = un.toString().split("e|E");
		String des = nums[0];
		UNumber temp = new UNumber(des.substring(1));
		UNumber ten = new UNumber("10");
		...
		else if (Math.abs(exp%2) == 0) {
			exp -= 2;
			temp.mpy(ten);
			temp.mpy(ten);
			exp /= 2;
		}
		String expo = "0.1e" + (exp+1);
		double gd = temp.getDouble();
		int i  = (int) gd;
		UNumber est = new UNumber(un.estimate(i));
		est.mpy(new UNumber(expo));
		return est;
	}
